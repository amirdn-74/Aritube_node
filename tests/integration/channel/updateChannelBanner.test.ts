...
describe("change channel name", () => {
  ...
  const exec = () => {
    return request(server)
      .put(`/api/channel/my/banner`)
      .set("x-auth-token", token)
      .attach("bannerPicture", image);
  };
  ...
  it("should return 400 if image is not set", async () => {
    ...
    expect(res.body.inner[0].path).toBe("bannerPicture");
  });
  ...
  it("should update channel collection in db", async () => {
    ...
    const updatedChannel = <IChannel>res.body;
    const now = Date.now();
    const fileName = +updatedChannel.banner.split(".")[0];
